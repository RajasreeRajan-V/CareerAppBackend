   ...
   public function index(Request $request)
{
    ...
    ->when($normalizedSearch, function ($query) use ($normalizedSearch, $normalizeCol) {
        $query->where(function ($q) use ($normalizedSearch, $normalizeCol) {
            $q->whereRaw($normalizeCol('title') . " LIKE ?", ["%{$normalizedSearch}%"])
              ->orWhereRaw($normalizeCol('description') . " LIKE ?", ["%{$normalizedSearch}%"])
              ->orWhereRaw($normalizeCol('specialization') . " LIKE ?", ["%{$normalizedSearch}%"])
              ->orWhereRaw($normalizeCol('subjects') . " LIKE ?", ["%{$normalizedSearch}%"])
              ->orWhereRaw($normalizeCol('career_options') . " LIKE ?", ["%{$normalizedSearch}%"]);
        })
        ->orderByRaw("
            CASE
                WHEN " . $normalizeCol('title') . " LIKE ? THEN 0
                WHEN " . $normalizeCol('subjects') . " LIKE ? THEN 1
                WHEN " . $normalizeCol('career_options') . " LIKE ? THEN 2
                ELSE 3
            END
        ", [
            "%{$normalizedSearch}%",
...
public function store(Request $request)
{
    $request->validate([
        ...
        'thumbnail' => 'nullable|image|mimes:jpg,jpeg,png,gif|max:3072',
    ...
    $career->growth         = $request->growth;
    $career->demand         = $request->demand;
...
public function update(Request $request, string $id)
{
    ...
    $request->validate([
        ...
        'thumbnail' => 'nullable|image|mimes:jpg,jpeg,png,gif|max:3072',
        'specialization'   => 'nullable|string',
        'growth'           => 'nullable|string',   // ADD
        'demand'           => 'nullable|string',   // ADD
        'remove_thumbnail' => 'nullable|string',
        'newgen_course'    => 'required|in:0,1',
    ]);
    ...
    $hasExistingMedia  = $career->video || $career->thumbnail;
   
    if (!$willHaveVideo && !$willHaveNewThumb && !$willKeepOldThumb && !$hasExistingMedia) {
        return back()->withErrors([
            'video' => 'Please provide a YouTube video URL or upload a thumbnail image.',
        ])->withInput();
    }
    ...
    $career->update([
        ...
        'subjects'       => $request->filled('subjects') ? json_encode(array_filter($request->subjects)) : null,
        'career_options' => json_encode($request->career_options),
        'video'          => $videoId,
        'thumbnail'      => $thumbnail,
        'specialization' => $request->specialization,
        'growth'         => $request->growth,    // ADD
        'demand'         => $request->demand,    // ADD
        'newgen_course'  => $request->newgen_course,
    ]);
