...
<div class="max-w-4xl mx-auto"
     x-data="{
       showModal:       {{ $errors->any() && old('_modal') === 'edit'   ? 'true' : 'false' }},
       showAddModal:    {{ $errors->any() && old('_modal') === 'add'    ? 'true' : 'false' }},
       showDeleteModal: false,
        courseId: {{ old('_course_id', 'null') }},
        courseName: '{{ old('name', '') }}',
        ...
        <div @click.away="showModal = false"
            ...
            <form method="POST" :action="`{{ route('college.collegeCourse.update', ':id') }}`.replace(':id', courseId)">
                ...
                @method('PUT')
                 ...
                 <input type="hidden" name="_modal" value="edit">          {{-- ADD THIS --}}
    <input type="hidden" name="_course_id" :value="courseId"> 
                <div class="mb-4">
    ...
    {{-- ================= ADD MODAL ================= --}}
    <div x-show="showAddModal" x-cloak x-transition
     class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
     ...
     class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        ...
        <div @click.away="showDeleteModal = false"
           ...
           <form method="POST" :action="`{{ route('college.collegeCourse.destroy', ':id') }}`.replace(':id', courseId)">
                @csrf
                @method('DELETE')
                <input type="hidden" name="_modal" value="delete">
                <div class="flex justify-end space-x-2">
                    <button type="button"
