            ...
            <div class="form-grid">
                    ...
                    <div class="form-card-header">
                        <span class="form-card-title">
                            <i class="fa-solid fa-circle-info"
                                style="color:var(--brand); margin-right:6px; font-size:.85rem"></i>
                            Fee Details
                        </span>
                    </div>
                    <div class="form-card-body">
                        ...
                        <div class="field-group">
                            ...
                            @php
                                $feeTypes = [
                                    ...
                                    'nri' => ['icon' => 'fa-earth-asia', 'label' => 'NRI', 'desc' => 'NRI quota'],
                                ];
                            @endphp

                            <div class="fee-type-group">
                                @foreach ($feeTypes as $value => $meta)
                                    @php $isUsed = in_array($value, $usedFeeTypes ?? []); @endphp

                                    <label class="fee-type-option {{ $isUsed ? 'fee-type-used' : '' }}">
                                        <input type="radio" name="fee_type" value="{{ $value }}"
                                            {{ old('fee_type') === $value ? 'checked' : '' }}
                                            {{ $isUsed ? 'disabled' : '' }}
                                            @if ($loop->first && !$isUsed && !old('fee_type')) checked @endif required>
                                        ...
                                        <div class="fee-type-card">
                                            ...
                                            <div class="type-desc">
                                                @if ($isUsed)
                                                    <span style="color:#ef4444; font-weight:600;">Already added</span>
                                                @else
                                                    {{ $meta['desc'] }}
                                                @endif
                                            </div>
                            ...
                            {{-- Block form entirely if all 3 types are used --}}
                            @if (count($usedFeeTypes ?? []) >= 3)
                                <div class="alert-danger" style="margin-top:12px;">
                                    <i class="fa-solid fa-circle-xmark" style="margin-right:6px;"></i>
                                    All fee types (Government, Management, NRI) have already been added for this course.
                                    No more fee structures can be added.
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="field-group">
                            ...
                            <div class="pill-group">
                                ...
                                <label class="pill-option">
                                    ...
                                    <span>
                                        <i class="fa-solid fa-calendar"
                                            style="margin-right:5px; font-size:.75rem"></i>Yearly
                                    </span>
                                </label>
                                ...
                                <label class="pill-option">
                                    ...
                                    <span>
                                        <i class="fa-solid fa-calendar-half"
                                            style="margin-right:5px; font-size:.75rem"></i>Semester
                                    </span>
                                </label>
                        ...
                        <div class="field-group">
                            ...
                            <select name="currency" class="form-select @error('currency') is-invalid @enderror" required>
                                @foreach ($currencies as $code => $label)
                                    ...
                                    @endif
                                    <option value="{{ $code }}"
                                        {{ old('currency', 'INR') == $code ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="field-group">
                            ...
                            <div class="input-addon-group">
                                ...
                                <input type="number" name="total_amount" :value="totalAmount"
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        ...
                        <button type="button" @click="addRow()" class="btn-back"
                            style="border-style:dashed; color:var(--brand); border-color:var(--brand-mid);">
                                ...
                                <div class="breakdown-row">
                                       ...
                                       {{-- Label (→ college_fee_breakdowns.label) --}}
                                            <div style="flex:1; min-width:0; display:flex; flex-direction:column; gap:2px;">
                                                <input type="text" :name="`breakdowns[${index}][label]`" x-model="row.label"
                                                    class="form-input" placeholder="e.g. Tuition Fee"
                                                    maxlength="255" required>
                                                <span class="field-hint"
                                                    style="text-align:right; line-height:1; margin-top:2px;"
                                                    x-text="`${(row.label || '').length}/255`"
                                                    :class="(row.label || '').length >= 240 ? 'invalid-msg' : 'field-hint'"
                                                    :style="'text-align:right; line-height:1; margin-top:2px;'">
                                                </span>
                                            </div>

                                    {{-- Amount --}}
                                        <div style="display:flex; flex-direction:column; width:136px; flex-shrink:0;">
                                            <div class="input-addon-group">
                                                <span class="input-addon"
                                                    style="font-size:.8rem; font-weight:600; color:var(--brand-dark);">₹</span>
                                                <input type="number" :name="`breakdowns[${index}][amount]`"
                                                    x-model.number="row.amount" @input="calcTotal()" class="form-input"
                                                    :class="row.exceeded ? 'is-invalid' : ''"
                                                    placeholder="0" min="0" max="100000000" step="0.01" required>
                                            </div>
                                            <template x-if="row.exceeded">
                                                <span style="font-size:.68rem; color:#ef4444; margin-top:3px; line-height:1.2;">
                                                    <i class="fa-solid fa-circle-exclamation" style="margin-right:2px;"></i>
                                                    Max ₹10 Crore exceeded
                                                </span>
                                            </template>
                                        </div>

                                    {{-- Sequence (→ college_fee_breakdowns.sequence) --}}
                                    <input type="number" :name="`breakdowns[${index}][sequence]`"
                                        x-model.number="row.sequence" class="form-input row-seq-input" min="1"
                                        :value="index + 1">

                                    {{-- Remove --}}
                                    <button type="button" @click="removeRow(index)" class="btn-remove-row"
                                        :disabled="breakdowns.length === 1" title="Remove row">
