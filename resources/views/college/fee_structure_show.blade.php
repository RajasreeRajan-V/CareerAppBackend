...
<style>
    ...
    .fee-card {
        ...
        display: flex; flex-direction: column;  /* ← ADD THIS */
    }
...
.fee-card-header {
    ...
    .fee-card-body { padding: 16px 18px; flex: 1; }  

    .breakdown-table { width: 100%; border-collapse: collapse; font-size: .83rem; table-layout: fixed; }
    .breakdown-table th {
                            ...
                            <tbody>
                                @foreach($fee->breakdowns as $breakdown)
                                    <tr>
                                        ...
                                        <td class="label-cell">{{ $breakdown->label }}</td>
...
(function () {
   ...
   window.recalcTotal = function () {
    ...
    document.querySelectorAll('.bd-amount').forEach(inp => {
        ...
        if (val > 100000000) inp.value = 100000000;
        total += parseFloat(inp.value) || 0;  // sum after clamping
    });
