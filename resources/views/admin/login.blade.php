       ...
       <div class="form-panel {{ session('login_type') === 'college' ? '' : 'active' }}" id="panel-admin">
            <form method="POST" action="{{ route('admin.do.login') }}">
                @csrf
                            {{-- Validation errors --}}
@if ($errors->any())
    <div style="background:#fff0f0; border:1px solid #ffb3b3; border-radius:8px; padding:12px 14px; margin-bottom:18px;">
        <ul style="margin:0; padding-left:18px; color:#8b0000; font-size:13px;">
...
@if (session('error'))
    <div style="background:#fff0f0; border:1px solid #ffb3b3; border-radius:8px; padding:12px 14px; margin-bottom:18px; color:#8b0000; font-size:13px;">
...
@endif
        ...
        <div class="form-panel {{ session('login_type') === 'college' ? 'active' : '' }}" id="panel-college">
            <form method="POST" action="{{ route('college.do.login') }}">
                @csrf
                {{-- Validation errors --}}
        @if ($errors->any() && session('login_type') === 'college')
            <div style="background:#fff0f0; border:1px solid #ffb3b3; border-radius:8px; padding:12px 14px; margin-bottom:18px;">
                <ul style="margin:0; padding-left:18px; color:#8b0000; font-size:13px;">
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
        ...
        {{-- Login failed message --}}
        @if (session('error') && session('login_type') === 'college')
            <div style="background:#fff0f0; border:1px solid #ffb3b3; border-radius:8px; padding:12px 14px; margin-bottom:18px; color:#8b0000; font-size:13px;">
        ...
        @endif
                <div class="form-group">
                    ...
                    <input type="email" name="email" placeholder="college@email.com"
             ...
             <div class="footer-text">
                Forgot your password? <a href="{{route('college.password.request')}}">Reset here</a>
            </div>
        </div>
    ...
    <script>
        ...
        {{-- Auto-switch to college tab if redirected back with college errors --}}
        // @if(session('login_type') === 'college' || (old('login_type') === 'college'))
        //     switchTo('college');
        // @endif
